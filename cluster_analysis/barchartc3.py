...
selected_columns = ['Neighborhood.Name','<18', 'adults', 'seniors', 'Clusters Category']
df = df[selected_columns]
print(df)
df = (df[df['Clusters Category'] == 'Cluster 3'])


df = df.groupby('Neighborhood.Name')[['<18', 'adults','seniors']].sum().reset_index()
print(df.shape[0])

df.set_index('Neighborhood.Name', inplace=True)


# # Create the stacked bar chart
df.plot(kind='barh', stacked=True,figsize=(4, 4))
